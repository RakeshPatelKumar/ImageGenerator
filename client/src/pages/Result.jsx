...
const Result = () => {
  ...
  src={image} 
  alt="Generated" 
  className="w-full max-w-sm h-[300px] object-cover rounded" 
/>

  <span 
    className={`absolute bottom-0 left-0 h-1 bg-red-500 transition-all duration-[10s] ease-in-out ${loading ? 'w-full' : 'w-0'}`} 
  />
</div>

      <p className={`${!loading ? 'hidden' : 'text-gray-300'}`}>Generating.....</p>
...
{isImageLoaded && 
    ...
    <p onClick={()=>{setIsImageLoaded(false)}} className='bg-transparent border border-gray-900 text-gray-300 px-8 py-3 rounded-full cursor-pointer'>Generate Another</p>
